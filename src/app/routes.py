"""
Author(s): Mckenna, Andrew, and Kobe
Description: The methods in this file map
URLs to functions, define helper functions,
process form input, handle user authorization,
and manage input validation. It also
implements business logic, queries the database,
and controls the application's responses.
"""
...
def get_or_create_cart(user):
    ...
    if cart is None:
        ...
        db.session.add(cart)
        db.session.flush()  # Ensures cart.id exists
...
def add_or_manage_cart_item(cart, plant_id: int, quantity: int):
    ...
    if cart_item:
        ...
        # Make sure the total in cart does not exceed stock before
        # adding to cart
...
def add_to_cart():
    ...
    # Validate quantity selected against stock
...
# Ensures the items in the cart have a non-zero quantity
...
def fix_item_quantity_before_checkout(cart):
    for item in cart.cart_items:
        # If the stock is gone, remove the item from the cart
        if item.plant.quantity <= 0:
            db.session.delete(item)

        # If the item quantity at checkout exceeds the actual stock quantity
        # then set the cart item quantity to be the stock quantity
        elif item.quantity > item.plant.quantity:
...
# Queries plants by the seller ID
...
# The seller dashboard shows their plant listings and competitor plant listings
